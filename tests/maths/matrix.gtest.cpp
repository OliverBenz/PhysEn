...
TEST(Matrix, Construction){
	{   // unity matrix
		matD<5, 5> matrixUnity(matrixType::unity);
        ...
        auto constr = [](){
            matD<2, 3> value(matrixType::unity);
        };
		EXPECT_THROW(constr(), std::invalid_argument);
	}

	{   // zero matrix
		matD<5, 5> matrixZero(matrixType::zero);
...
TEST(Matrix, Submatrix){
	{  // 4x4 matrix
		...
		matD<3, 3> resultExpected{
            {1, 2, 3},
            {3, 6, 7},
            {1, 4, 7}
		};

		matD<3, 3> result = mat.getSubMatrix(0, 3);
...
#pragma GCC diagnostic pop
	...
	{ // Equality operator
		...
		// Same matrix
		matD<2, 3> second = first;
		EXPECT_TRUE(first == second);
		EXPECT_FALSE(first != second);

		// Small difference
		matD<2, 3> third = second;
